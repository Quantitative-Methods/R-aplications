...
colnames(ds1) <- c("N", "MEAN", "MEDIAN", "SD", "MIN", "MAX", "RANGE", "SKEW", "KURT")
ds2 <- ds2[1:m+1, c(2,3,5,4,8,9,10,11,12)]
colnames(ds2) <- c("N", "MEAN", "MEDIAN", "SD", "MIN", "MAX", "RANGE", "SKEW", "KURT")
```
...
server <- function(input, output, session) {
      ...
      selectedData2 <- reactive({   
       ...
       output$des <- renderDataTable({
        ...
        colnames(des) <- c("GROUPS", "N", "MEAN", "SD", "SEE", "CI", "Shapiro-Wilk W", "P")
        des%>% DT::datatable (rownames = FALSE, extensions = 'Buttons', options = list(pageLength = 100, dom = 'Bt',buttons = c('copy'))) %>% formatRound(c("MEAN", "SD", "SEE", "CI", "Shapiro-Wilk W", "P"), 2)
       })
...
ui <- fluidPage(
        ...
        mainPanel(
          ...
          hr(),
          h6("*Izvorni Leveneov test homogenosti varijanci računa odstupanja rezultata entiteta od aritmetičke sredine. Brown i Forsythe (1974.) su predložili da se umjesto aritmetičke sredine koristi medijan jer je manje osjetljiv (bolja mjera centralne tendencije) u slučaju asimetričnih distribucija podataka."),
         h6("*Original Levene test of homogeneity of variance of calculus of deviation of entity results from arithmetic mean. Brown and Forsythe (1974) suggested that the median be used instead of the arithmetic mean because it is less sensitive (a better measure of central tendency) in the case of asymmetric data distributions.")
        )
