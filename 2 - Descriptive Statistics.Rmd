...
server <- function(input, output, session) {
      ...
      output$asp <- renderPlot({
        xx <- selectedData()
          ...
          polygon(c(x1, a, x2), c(0, b, 0), col="#8ecae6")
      }) 
...
```{r}
DT::datatable (P, extensions = 'Buttons', options = list(pageLength = m, dom = 'Bt',buttons = c('copy'))) %>% formatRound(colnames(P), digits=4) %>% formatStyle(var_name_numeric, color = styleInterval(c(0.05), c('red', 'black')))
```
...
server <- function(input, output) {
    ...
    numeric %>% correlate() %>% network_plot(min_cor=input$r)
    })
},
shinyApp(ui = ui, server = server)
)
