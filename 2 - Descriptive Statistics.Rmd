...
# PODACI: oznake (kodovi) modaliteta kvalitativnih varijabli ne smiju biti brojevi
# SELEKCIJA VARIJABLI: aplikacija samostalno vrši selekciju svih kvantitativnih varijabli
```
...
```{r}
DT::datatable(data, style = 'bootstrap', filter = 'top', extensions = 'Buttons', options = list(pageLength = n, dom = 'Brti', buttons = c('copy','csv', 'excel'))) 
```
...
server <- function(input, output, session) {
      ...
      output$box <- renderPlot({
        ...
        #geom_violin (color="#5BC0DE", fill="#5BC0DE", alpha=0.7) +
...
ui <- fluidPage(
        ...
        mainPanel(
          #h4("Confidence Intervals for the Population Mean"),
          htmlOutput("ci"),
          plotOutput("asp")
          )
),
