...
# Probability Calculator #
	...
	output$grafnd <- renderPlot({
	  x <- seq(-4,4, 0.01)*input$sd1 + input$as1
	  px <- dnorm(x, input$as1, input$sd1)
	  dfnd <- cbind.data.frame(x,px)
	    fn <- function(x) {
	      y <- dnorm(x, mean = input$as1, sd = input$sd1)
	      y[x < input$rez1 | x > (max(x))] <- NA
	      return(y)
	    }
	  ggplot(data=dfnd, aes(x=x, y=px)) +
	    ...
	    xlab("x value") +
	    ylab("p(x)") +
	    geom_text(x = input$rez1, y = -0.0006, label =  paste0(round(input$rez1, digits = 2))) +
	    theme_classic()
	})
