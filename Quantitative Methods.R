			...
			dashboardSidebar(
				sidebarMenu(
					...
					menuItem("Basic Statistics",
						...
						menuSubItem("Probability Calculator", tabName = "PC"),
				...
				tabItem(tabName = "PC",
					fluidRow(
						box(title = "Napomena!",
							...
							collapsed = TRUE,
							width = 12,
							p("Kalkulator vjerojatnosti omogucava racunanje:"),
							p(" - vjerojatnosti postizanja boljeg ili losijeg rezultata (P) od rezultata (X) u populaciji entiteta 
							cije obiljezje (varijabla) ima normalnu ili Gaussovu distribuciju s odredenom aritmetickm sredinom (Mean) 
							i standardnom devijacijom (Standard Deviation)  i "),
							p(" - vjerojatnosti postizanja rezultata koji su bolji od rezultata X1 i losiji od rezultata X2 u 
							populaciji entiteta cije obiljezje (varijabla) ima normalnu ili Gaussovu distribuciju s odredenom 
							aritmetickm sredinom (Mean) i standardnom devijacijom (Standard Deviation).")
						),
						box(title = "Normal (Gauss) Distribution - Probability Calculator",
							status = "primary",
							solidHeader = TRUE,
							collapsible = TRUE,
							width = 6,
							numericInput("as1", "Mean", 180),
							verbatimTextOutput("as1"),
							numericInput("sd1", "Standard Deviation", 10),
							verbatimTextOutput("sd1"),
							numericInput("rez1", "Value (X)", 190),
						...
						box(title = "Normal (Gauss) Distribution - Probability Calculator",
							...
							collapsible = TRUE,
							width = 6,
							numericInput("as2", "Mean", 180),
							verbatimTextOutput("as2"),
							numericInput("sd2", "Standard Deviation", 10),
							verbatimTextOutput("sd2"),
							numericInput("rez11", "First Value (X1)", 155),
							verbatimTextOutput("rez11"),
							numericInput("rez12", "Second Value (X2)", 194),
							verbatimTextOutput("rez12"),
							plotOutput("graf13"),
							plotOutput("graf14")
						)
					)
...
server <- function(input, output) { 
	...
	# Probability Calculator #
		...
		pnormGC(input$rez1, region="above", mean=input$as1,
		sd=input$sd1, graph=TRUE)
	})
	output$graf12 <- renderPlot({
		pnormGC(input$rez1,region="below",mean=input$as1,
		sd=input$sd1,graph=TRUE)
	})
	output$graf13 <- renderPlot({
		pnormGC(c(input$rez11, input$rez12), region="between", mean=input$as2,
		sd=input$sd2, graph=TRUE)
	})
	output$graf14 <- renderPlot({
		pnormGC(c(input$rez11, input$rez12), region="outside", mean=input$as2,
		sd=input$sd2, graph=TRUE)
	})
